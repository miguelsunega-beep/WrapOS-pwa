...
- **RLS (Row Level Security)**: `supabase/policies.sql` é a fonte da verdade das políticas de RLS para as 13 tabelas. Padrão confirmado em produção (inspecionado, não suposição): policy `FOR ALL TO public`, nome `{tabela}_por_loja` (exceto `ordens_servico`, cuja policy chama `ordens_por_loja`), `USING ("lojaId" IN (SELECT usuarios."lojaId" FROM usuarios WHERE usuarios."authUserId" = auth.uid()::text))` — subquery direta na tabela `usuarios`, sem função auxiliar, com cast explícito de `auth.uid()` pra `::text`. Confirmado pra `clientes_por_loja`, `veiculos_por_loja`, `ordens_por_loja` e `usuarios_por_loja`; a única exceção não confirmada é a policy de `lojas` (reconstrução de melhor esforço, sinalizada no próprio arquivo). Igual às migrations, **aplicar esse arquivo não é automático** — sempre que uma policy mudar (aqui ou direto no painel do Supabase), atualize os dois lados manualmente para não ficarem dessincronizados. Nas 8 tabelas novas da Fase 1.1, RLS já foi habilitado via Dashboard — falta só aplicar as policies deste arquivo.
