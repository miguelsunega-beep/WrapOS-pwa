...
- **RLS (Row Level Security)**: `supabase/policies.sql` é a fonte da verdade das políticas de RLS para as 13 tabelas (isolamento multi-tenant por `lojaId`, via função auxiliar `current_loja_id()` que resolve `auth.uid()` → `usuarios.lojaId`). Igual às migrations, **aplicar esse arquivo não é automático** — sempre que uma policy mudar (aqui ou direto no painel do Supabase), atualize os dois lados manualmente para não ficarem dessincronizados. As policies das 5 tabelas originais já estavam configuradas no painel antes deste arquivo existir e nunca haviam sido versionadas; o conteúdo de `policies.sql` pra elas é uma reconstrução de melhor esforço do padrão assumido pelo app, não uma cópia garantida do que está aplicado — se divergir do painel, o painel manda até `policies.sql` ser corrigido.
